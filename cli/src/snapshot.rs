            ...
            if let Ok(v_resp) = client.get(&versions_url).send().await {
                if v_resp.status().is_success() {
                    if let Ok(v_data) = v_resp.json::<serde_json::Value>().await {
                        let v_items = v_data.get("items").and_then(|v| v.as_array()).cloned().unwrap_or_default();
...
pub async fn sign(snapshot_file: &str, key_path: &str) -> Result<()> {
    ...
    let mut snapshot: SignedSnapshot = serde_json::from_str(&snapshot_content)
...
pub async fn verify(snapshot_file: &str, trust_key_path: &str) -> Result<()> {
    ...
    let snapshot_content = fs::read_to_string(snapshot_file)?;
    let snapshot: SignedSnapshot = serde_json::from_str(&snapshot_content)
        .context("Failed to parse snapshot file")?;

    let signature_b64 = snapshot.signature.as_ref().context("Snapshot has no signature")?;
...
pub async fn inspect(snapshot_file: &str) -> Result<()> {
    ...
    println!("  {}: {}", "Identity".bold(), snapshot.payload.registry_identity);
    println!("  {}: {}", "Timestamp".bold(), snapshot.payload.timestamp);
    println!("  {}: {}", "Contracts".bold(), snapshot.payload.contracts.len());
    println!("  {}: {}", "Versions".bold(), snapshot.payload.versions.len());
